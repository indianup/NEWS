...
async def process_txt(bot: Client, m: Message):
    editable = await m.reply_text("**Send Your TXT file with links**")
    input_file_message: Message = await bot.listen(editable.chat.id)
    txt_file = await input_file_message.download()
    await input_file_message.delete()
    await editable.delete()
    ...
    editable = await m.reply_text(
        ...
        "1: Remove lines containing a specific word\n"
        "2: Remove a specific word from every line\n"
        "3: Replace a word with another word\n"
        "4: Sort the lines naturally\n"
        "Send the corresponding number:"
    )
    choice_message: Message = await bot.listen(editable.chat.id)
    choice = choice_message.text.strip()
    await choice_message.delete()
    await editable.delete()

    if choice == "1":
        # Remove lines containing a specific word
        editable = await m.reply_text("**Send the word to remove lines containing it:**")
        word_message: Message = await bot.listen(editable.chat.id)
        word_to_remove = word_message.text.strip()
        await word_message.delete()
        await editable.delete()

        cleaned_lines = [line for line in lines if word_to_remove not in line]

    elif choice == "2":
        # Remove a specific word from every line
        editable = await m.reply_text("**Send the word to remove from every line:**")
        word_message: Message = await bot.listen(editable.chat.id)
        word_to_remove = word_message.text.strip()
        await word_message.delete()
        await editable.delete()

        cleaned_lines = [line.replace(word_to_remove, '') for line in lines]

    elif choice == "3":
        # Replace a word with another word
        editable = await m.reply_text("**Send the word you want to replace:**")
        old_word_message: Message = await bot.listen(editable.chat.id)
        old_word = old_word_message.text.strip()
        await old_word_message.delete()

        editable = await m.reply_text("**Send the new word to replace it with:**")
        new_word_message: Message = await bot.listen(editable.chat.id)
        new_word = new_word_message.text.strip()
        await new_word_message.delete()
        await editable.delete()

        cleaned_lines = [line.replace(old_word, new_word) for line in lines]

    elif choice == "4":
        # Sort lines naturally
        cleaned_lines = sorted(lines, key=natural_sort_key)

    else:
        await m.reply_text("Invalid choice. Please start again.")
        os.remove(txt_file)
        return

    # Save the cleaned file
    cleaned_txt_file = os.path.splitext(txt_file)[0] + '.txt'
    with open(cleaned_txt_file, 'w') as f:
        f.writelines(cleaned_lines)

    # Send the cleaned file back to the user
    await m.reply_document(document=cleaned_txt_file, caption="Here is your processed txt file.")
...
#===============================================================
    if raw_text5 == "df":
        await editable.edit("**-═════━‧₊˚❀༉‧₊˚.━═════-\nᴇɴᴛᴇʀ ʙᴀᴛᴄʜ ɴᴀᴍᴇ ᴏʀ ꜱᴇɴᴅ `df` ꜰᴏʀ ɢʀᴀʙɪɴɢ ꜰʀᴏᴍ ᴛᴇxᴛ ꜰɪʟᴇɴᴀᴍᴇ.\n-═════━‧₊˚❀༉‧₊˚.━═════-**")
    ...
    #===================== Title Name =====================
        ...
        await editable.edit(f"Who is Your **Father**")
    ...
    #===================== QUALITY =================================
        ...
        await editable.edit("**-═════━‧₊˚❀༉‧₊˚.━═════-\nᴇɴᴛᴇʀ ʏᴏᴜʀ ɴᴀᴍᴇ ᴏʀ ꜱᴇɴᴅ `df` ꜰᴏʀ ᴜꜱᴇ ᴅᴇꜰᴀᴜʟᴛ\n-═════━‧₊˚❀༉‧₊˚.━═════-**")
        ...
        await editable.edit("-═════━‧₊˚❀༉‧₊˚.━═════-\nɴᴏᴡ ꜱᴇɴᴅ ᴛʜᴇ **ᴛʜᴜᴍʙ ᴜʀʟ** ᴏʀ ꜱᴇɴᴅ [`no`]\n-═════━‧₊˚❀༉‧₊˚.━═════-")
